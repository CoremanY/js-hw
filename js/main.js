...
let wordsList = (str, subStr) => {
    ...
    let lowerCaseCleaner = str
    .replace(/[.,!?;:"]/g, '')
    .toLowerCase()
    .split(' ')
    .filter((lowerCaseCleanerItem) => lowerCaseCleanerItem.indexOf(subStr) > -1);
  
    let res = new Set();

    lowerCaseCleaner.forEach((lowerCaseCleanerItem) => {res.add(lowerCaseCleanerItem);});
